...
// https://github.com/Almenon/renderjson
...
var module,
	...
	renderjson = (function () {
		const themetext = function (...args: string[] /* [class, text]+ */) {
			...
			while (args.length)
				spans.push(
					append(span(Array.prototype.shift.call(args)), text(Array.prototype.shift.call(args)))
				);
			return spans;
		};
		const append = function (...args: (Node | null | Node[])[] /* el, ... */) {
			...
			for (let a = 0; a < args.length; a++)
				...
				else el.appendChild(<Node>args[a]);
			return el;
		};
		...
		const A = function A(txt: string, classname: string, callback: Function) {
		...
		function _renderjson(
			...
			if (json.constructor == Array) {
				...
				return disclosure(
					'[',
					options.collapse_msg((<any[]>json).length),
					']',
					'array',
					function () {
						...
						for (let i = 0; i < (<any[]>json).length; i++)
							append(
								as,
								_renderjson(
									options.replacer.call(json, i, (<any[]>json)[i]),
									indent + '    ',
									false,
									show_level - 1,
									options
		...
		renderjson.set_collapse_msg(function (len: number) {
