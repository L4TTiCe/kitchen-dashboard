<script lang="ts">
	...
	async function getFoodById(id: string) {
		await axios
			.get(variables.SERVER_URL + '/food/' + id)
			.then((response) => {
				food = response.data;
			})
			.catch((error) => {
...
<div class="flex flex-wrap -mx-3 mb-6">
	<div class="w-full px-3">
		...
		<select
			...
			class="{!owner_id ? 'border-red-500' : 'border-gray-200'} {users_data.length == 0
				? 'disabled'
				: ''} block w-full bg-gray-200 text-gray-700 border rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white focus:border-gray-500"
			id="grid-owner"
		>
			<option value={null} selected={!owner_id} disabled hidden
				>{users_data.length == 0 ? 'No Users in this Group' : 'Choose User'}</option
			>
...
<div class="flex flex-wrap -mx-3 mb-6">
	<div class="w-full px-3">
		...
		<select
			bind:value={food_id}
			disabled={disable || food_data.length == 0}
			class="{!food_id
				? 'border-red-500'
		...
		>
			<option value={null} selected disabled hidden
				>{food_data.length == 0 ? 'No Food' : 'Choose Food'}</option
			>
	...
	<div class="w-full px-3">
		...
		>
			<option value={null} disabled hidden
				>{location_data.length == 0 ? 'No Available Locations' : 'Select Location'}</option
			>
	...
	<div class="px-3">
		<button
			...
			class="{disable
				...
				: 'bg-blue-500 hover:bg-blue-400'}block shadow tracking-wide focus:shadow-outline focus:outline-none text-white font-bold py-2 px-4"
			type="button"
		>
			{button_text}
		</button>
