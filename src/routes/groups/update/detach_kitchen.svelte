<script lang="ts">
    ...
    async function getGroups() {
        await axios.get(variables.SERVER_URL + '/groups')
            .then(response => {
    ...
    async function getKitchens() {
        await axios.get(variables.SERVER_URL + '/kitchens')
            .then(response => {
    ...
    function handleSubmit() {
        if (selected_group) {
            ...
            axios.put(variables.SERVER_URL + '/groups/ById/' + selected_group._id, group)
            .then(response => {
    ...
    <form class="w-full max-w-lg mx-4">
        <div class="flex flex-wrap -mx-3 mb-6">
            <div class="w-full px-3">
                ...
                <select bind:value={selected_group} 
                    class="{!selected_group ? "border-red-500" : "border-gray-200"} block w-full bg-gray-200 text-gray-700 border rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white focus:border-gray-500" 
                    id="grid-group" >
                ...
                {#if !selected_group}
                    <p class="text-red-500 text-xs italic ">Please choose group to detach the Kitchen from.</p>
        ...
        <div class="flex flex-wrap -mx-3 mb-6">
            <div class="w-full px-3">
                ...
                <input bind:value={selected_kitchen.name} 
                    type="text" 
