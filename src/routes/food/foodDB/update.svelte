...
<script lang="ts">
	...
	import FoodForm from '$lib/components/food/food_form.svelte';
	...
	function handleSubmit() {
		...
		if (canPost) {
			axios
				.put(variables.SERVER_URL + '/food/' + selected._id, food)
				.then((response) => {
	...
	<form class="w-full max-w-lg mx-4">
		<div class="flex flex-wrap -mx-3 mb-6">
			<div class="w-full px-3">
				...
				>
					<option value={null} selected>{food_data.length == 0 ? 'No Food' : 'Select Food'}</option>
		...
		{#if selected}
			<form class="w-full max-w-lg mx-4">
				<FoodForm bind:name bind:nutrition bind:upc bind:nutrition_data />
			</form>
				...
				<div class="p-3 w-full flex justify-center items-center">
					...
					{#if nutrition}
						<NutritionLabel
							...
							added_sugar={nutrition.added_sugar}
