<script lang="ts">
    ...
    async function getFoodData() {
        await axios.get(variables.SERVER_URL + '/food')
            .then(response => {
...
<div>
    ...
    <form class="w-full max-w-lg mx-4">
        ...
        <OwnershipForm
            ...
            bind:quantity_unit={quantity_unit}
            bind:procured={procured}
            bind:expiry={expiry}
            bind:location={location}

            bind:status_message={status_message}
            bind:users_data={users_data}
            bind:food_data={food_data}
            bind:location_data={location_data}

            handleSubmit={handleSubmit}
            button_text={"Remove Food from Inventory"}
            disable={true}
        />
        {/if}
        {/if}
