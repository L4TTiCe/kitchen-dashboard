<script lang="ts">
    ...
    function handleSubmit() {
        ...
        if (canPost) {
            axios.delete(variables.SERVER_URL + '/locations/' + selected_location._id)
                .then(response => {
...
<div>
    ...
    {#if selected_kitchen}
        ...
        <div class="content-center -mx-3 mb-6">
            <div class="px-3">
                <button on:click={handleSubmit} class="block shadow tracking-wide bg-red-500 hover:bg-red-400 focus:shadow-outline focus:outline-none text-white font-bold py-2 px-4" 
                    type="button">
                    Delete Location
                </button>
                {#if status_message != ''}
