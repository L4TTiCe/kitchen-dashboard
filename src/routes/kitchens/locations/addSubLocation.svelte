<script lang="ts">
    ...
    async function getLocationData() {
        axios.get(variables.SERVER_URL + '/kitchens/' + selected_kitchen._id)
        .then(response => {
    ...
    function handleSubmit() {
        ...
        if (canPost) {
            axios.post(variables.SERVER_URL + '/locations/' + selected_location._id, location)
                .then(response => {
...
<div>
    <h2 class="text-2xl m-4">Add Sub-Location to a Location</h2>
    ...
    <form class="w-full max-w-lg mx-4">
        <div class="flex flex-wrap -mx-3 mb-6">
            <div class="w-full px-3">
                ...
                <select bind:value={selected_kitchen} class="{!selected_kitchen ? "border-red-500" : "border-gray-200"} block w-full bg-gray-200 text-gray-700 border rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white focus:border-gray-500" id="grid-username" placeholder="Choose group to update">
    ...
    {#if selected_kitchen}
        <div class="flex flex-wrap -mx-3 mb-6">
            <div class="w-full px-3">
                ...
                <select bind:value={selected_location} class="{!selected_location ? "border-red-500" : "border-gray-200"} block w-full bg-gray-200 text-gray-700 border rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white focus:border-gray-500" id="grid-username" placeholder="Choose group to update">
        ...
        <div class="flex flex-wrap -mx-3 mb-6">
            <div class="w-full px-3">
                ...
                <input bind:value={name} 
                    type="text" 
                    class="{name == '' ? "border-red-500" : "border-gray-200"} appearance-none block w-full bg-gray-200 text-gray-700 border rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500" 
                    id="grid-qty-u" 
        ...
        <div class="content-center -mx-3 mb-6">
            <div class="px-3">
                <button on:click={handleSubmit} class="block shadow tracking-wide bg-blue-500 hover:bg-blue-400 focus:shadow-outline focus:outline-none text-white font-bold py-2 px-4" 
                    type="button">
                    Add Sub-Location
                </button>
                {#if status_message != ''}
